...
class TransactionUtil extends Util
{
    ...
    public function updateCashDenominations($payment, $all_denominations)
    {
            ...
            foreach ($all_denominations as $key => $value) {
                if (! empty($value)) {
                    CashDenomination::updateOrCreate(
                        ['business_id' => $payment->business_id, 'model_id' => $payment->id, 'model_type' => TransactionPayment::class, 'amount' => $key],
                        ['total_count' => $value]
                    );
                    $denominations[] = $key;
                }
        ...
        CashDenomination::where('business_id', $payment->business_id)
            ->where('model_type', TransactionPayment::class)
        ...
        if (! empty($il->module_info['service_staff']) && $this->isModuleEnabled('service_staff')) {
            ...
            if (isset($il->module_info['service_staff']['show_service_staff'])) {
                ...
                if (! empty($transaction->res_waiter_id)) {
                    $waiter = User::find($transaction->res_waiter_id);
                }
        ...
        if (! empty($il->module_info['repair']) && $transaction->sub_type == 'repair') {
            if (! empty($il->module_info['repair']['show_repair_status'])) {
                ...
                if (! empty($transaction->repair_status_id)) {
                    $repair_status = RepairStatus::find($transaction->repair_status_id);
            ...
            if (! empty($il->module_info['repair']['show_repair_warranty'])) {
                ...
                if (! empty($transaction->repair_warranty_id)) {
                    $repair_warranty = Warranty::find($transaction->repair_warranty_id);
            ...
            if (! empty($il->module_info['repair']['show_model'])) {
                ...
                if (! empty($transaction->repair_model_id)) {
                    $device_model = DeviceModel::find($transaction->repair_model_id);
            ...
            if (! empty($il->module_info['repair']['show_repair_checklist'])) {
                ...
                $checklists = [];
                if (! empty($transaction->repair_model_id)) {
                    $model = DeviceModel::find($transaction->repair_model_id);
    ...
    public function getTotalSellDueRecovery($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = TransactionPayment::join('transactions as t', 't.id', 'transaction_payments.transaction_id')
            ->where('t.type', 'sell')
            ...
            ->whereRaw('DATE(transaction_payments.paid_on) > DATE(t.transaction_date)')
            ->select(
                DB::raw('SUM(transaction_payments.amount) as total_due_recovery')
            );
        ...
        $total_due_recovery = $query->first()->total_due_recovery ?? 0;

        return $total_due_recovery;
    }
    ...
    public function getSellsCurrentFy($business_id, $start, $end)
    {
        ...
        $sells = $query->select(
            ...
            ->groupBy(DB::raw("DATE_FORMAT(transactions.transaction_date, '%m-%Y')"))
            ->get();
