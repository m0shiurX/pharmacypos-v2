<div class="row pos_form_totals" style="border-top:2px solid #e0e0e0; margin-top:0; padding-top:8px">
	<div class="col-md-12">
		<table class="table table-condensed" style="margin-bottom:4px">
			<tr>
				<td><b class="tw-text-sm tw-font-semibold tw-text-gray-700">@lang('sale.item'):</b>&nbsp;
					<span class="total_quantity tw-text-sm tw-font-semibold">0</span></td>
				<td>
					<b class="tw-text-sm tw-font-semibold tw-text-gray-700">@lang('sale.total'):</b> &nbsp;
					<span class="price_total tw-text-sm tw-font-semibold">0</span>
				</td>
			</tr>
			<tr>
				
					<td @if(!Gate::check('disable_discount') || auth()->user()->can('superadmin') || auth()->user()->can('admin')) class="" @else class="hide" @endif>
						<b class="tw-text-sm tw-font-semibold tw-text-gray-700">
							...
							@if($is_discount_enabled)
								(-):
								@if($edit_discount)
								<i class="fas fa-edit cursor-pointer tw-text-gray-500" id="pos-edit-discount" title="@lang('sale.edit_discount')" aria-hidden="true" data-toggle="modal" data-target="#posEditDiscountModal"></i>
								@endif
							
								<span class="tw-text-sm tw-font-semibold" id="total_discount">0</span>
							@endif
				...
				<td class="@if($pos_settings['disable_order_tax'] != 0) hide @endif">
					<span class="tw-text-sm tw-font-semibold">
						<b class="tw-text-sm tw-font-semibold tw-text-gray-700">@lang('sale.order_tax')(+): @show_tooltip(__('tooltip.sale_tax'))</b>
						<i class="fas fa-edit cursor-pointer tw-text-gray-500" title="@lang('sale.edit_order_tax')" aria-hidden="true" data-toggle="modal" data-target="#posEditOrderTaxModal" id="pos-edit-tax" ></i> 
						<span class="tw-text-sm tw-font-semibold" id="order_tax">
							@if(empty($edit))
								0
							@else
				...
				<td>
					<span class="tw-text-sm tw-font-semibold">
						...
						<b class="tw-text-sm tw-font-semibold tw-text-gray-700">@lang('sale.shipping')(+): @show_tooltip(__('tooltip.shipping'))</b> 
						<i class="fas fa-edit cursor-pointer tw-text-gray-500"  title="@lang('sale.shipping')" aria-hidden="true" data-toggle="modal" data-target="#posShippingModal"></i>
				...
				@if(in_array('types_of_service', $enabled_modules))
					<td class="col-sm-3 col-xs-6 d-inline-table">
						<b class="tw-text-sm tw-font-semibold tw-text-gray-700">@lang('lang_v1.packing_charge')(+):</b>
						<i class="fas fa-edit cursor-pointer service_modal_btn tw-text-gray-500"></i> 
						<span  class="tw-text-sm tw-font-semibold" id="packing_charge_text">
							0
						</span>
					</td>
				@endif
				...
				<td>
					<b class="tw-text-sm tw-font-semibold tw-text-gray-700" id="round_off">@lang('lang_v1.round_off'):</b> <span id="round_off_text">0</span>								
