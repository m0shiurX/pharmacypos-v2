...
function render_sub_unit_multipliers() {
    var container = $('#sub_unit_multipliers_container');
    ...
    $('#sub_unit_ids option:selected').each(function () {
        var unit_id = $(this).val();
        var multiplier = $(this).data('multiplier');
        var unit_name = $(this).data('unit_name') || $(this).text();
        // Only show multiplier input for sub-units (multiplier != 1)
        if (multiplier && multiplier != 1) {
            has_fields = true;
            container.append(
                '<div class="col-sm-3 sub_unit_multiplier_row" data-unit_id="' + unit_id + '">' +
                '  <div class="form-group">' +
                '    <label>' + unit_name + ' ' + LANG.multiplier + ':</label>' +
                '    <input type="number" name="sub_unit_multipliers[' + unit_id + ']" class="form-control" value="' + multiplier + '" min="0.0001" step="any">' +
                '  </div>' +
                '</div>'
            );
        }
